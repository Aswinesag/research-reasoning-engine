...
export function QueryBar() {
  const { activeQuery, domain, depth, viewMode, setQuery, setDomain, setDepth, setViewMode, setResult } = useResearchStore()
  const mutation = useRunResearch()

  async function submit(event: FormEvent) {
    event.preventDefault()
    const result = await mutation.mutateAsync({ query: activeQuery, domain, depth, simplified: viewMode === 'simplified' })
    setResult(result)
  }
